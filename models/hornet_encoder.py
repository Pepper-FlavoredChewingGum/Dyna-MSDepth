...
class HorNet(nn.Module):
    ...
    def forward(self, x):
        self.check = self.forward_features(x)
        # x = self.head(x)
        return self.check
...
def hornet_tiny_7x7(pretrained=False,in_22k=False, **kwargs):
    s = 1.0/3.0
    model = HorNet(depths=[1, 1, 1, 1], base_dim=64, block=Block,
    gnconv=[
...
@register_model
def hornet_tiny_gf(in_chans=3, pretrained=False,in_22k=False, **kwargs):
    s = 1.0/3.0
    model = HorNet(in_chans=in_chans, depths=[2, 2, 2, 2], base_dim=64, block=Block,
    gnconv=[
        partial(gnconv, order=2, s=s),
        partial(gnconv, order=3, s=s),
