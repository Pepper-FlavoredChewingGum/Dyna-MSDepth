...
class DepthDecoder(nn.Module):
    def __init__(self, num_output_channels=1):
        ...
        # Conv layers
        self.conv_up = SeparableConvBlock(self.num_channels)
    ...
    def forward(self, input_features, HorBlock):
        ...
        p4_w0 = self.relu(self.p4_w0)
        weight = p4_w0 / (torch.sum(p4_w0, dim=0) + self.epsilon)
        p4_in = self.conv_up(self.swish(weight[0] * p4_in + weight[1] * HorBlock3))

        # Weights for P3_0 and Hor2 to P3_0
        p3_w0 = self.relu(self.p3_w0)
        weight = p3_w0 / (torch.sum(p3_w0, dim=0) + self.epsilon)
        p3_in = self.conv_up(self.swish(weight[0] * p3_in + weight[1] * HorBlock2))

        # Weights for P2_0 and Hor1 to P2_0
        p2_w0 = self.relu(self.p2_w0)
        weight = p2_w0 / (torch.sum(p2_w0, dim=0) + self.epsilon)
        p2_in = self.conv_up(self.swish(weight[0] * p2_in + weight[1] * HorBlock1))

        # Weights for P1_0 and Hor0 to P1_0
        p1_w0 = self.relu(self.p1_w0)
        weight = p1_w0 / (torch.sum(p1_w0, dim=0) + self.epsilon)
        p1_in = self.conv_up(self.swish(weight[0] * p1_in + weight[1] * HorBlock0))

        # Weights for P4_0 and P3_0 to P3_1
        p3_w1 = self.relu(self.p3_w1)
        weight = p3_w1 / (torch.sum(p3_w1, dim=0) + self.epsilon)
        p3_1 = self.conv_up(self.swish(weight[0] * p3_in + weight[1] * self.upsample(p4_in)))

        # Weights for P3_0 and P2_0 and p3_1 to P2_1
        p2_w1 = self.relu(self.p2_w1)
        weight = p2_w1 / (torch.sum(p2_w1, dim=0) + self.epsilon)
        p2_1 = self.conv_up(self.swish(weight[0] * p2_in + weight[1] * self.upsample(p3_in) + weight[2] * self.upsample(p3_1)))
        ...
        p1_1 = self.conv_up(self.swish(weight[0] * p1_in + weight[1] * self.upsample(p2_in) + weight[2] * self.upsample(self.upsample(p3_1))))

        # Weights for P1_1 and P0_0 to P0_1
        p0_w1 = self.relu(self.p0_w1)
        weight = p0_w1 / (torch.sum(p0_w1, dim=0) + self.epsilon)
        p0_1 = self.conv_up(self.swish(weight[0] * p0_in + weight[1] * self.upsample(p1_1)))

        # Weights for P4_0, P3_0, P3_1 to P3_2
        p3_w2 = self.relu(self.p3_w2)
        weight = p3_w2 / (torch.sum(p3_w2, dim=0) + self.epsilon)
        p3_2 = self.conv_up(self.swish(weight[0] * p3_in + weight[1] * p3_1 + weight[2] * self.upsample(p4_in)))

        # Weights for P2_0, P2_1, P3_2 to P2_2
        p2_w2 = self.relu(self.p2_w2)
        weight = p2_w2 / (torch.sum(p2_w2, dim=0) + self.epsilon)
        p2_2 = self.conv_up(self.swish(weight[0] * p2_in + weight[1] * p2_1 + weight[2] * self.upsample(p3_2)))

        # Weights for P1_0, P1_1, P2_2 to P1_2
        p1_w2 = self.relu(self.p1_w2)
        weight = p1_w2 / (torch.sum(p1_w2, dim=0) + self.epsilon)
        p1_2 = self.conv_up(self.swish(weight[0] * p1_in + weight[1] * p1_1 + weight[2] * self.upsample(p2_2)))

        # Weights for P0_0, P0_1 and P1_2 to P0_2
        p0_w2 = self.relu(self.p0_w2)
        weight = p0_w2 / (torch.sum(p0_w2, dim=0) + self.epsilon)
        BiFPN_output = self.conv_up(self.swish(weight[0] * p0_in + weight[1] * p0_1 + weight[2] * self.upsample(p1_2)))

        BiFPN_output = self.output1(BiFPN_output)
        BiFPN_output = self.upsample(BiFPN_output)
