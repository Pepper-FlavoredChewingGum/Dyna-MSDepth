...
class DepthDecoder(nn.Module):
    ...
    def forward(self, input_features, Hor_features):
        ...
        #output = torch.cat([self.outputs[3], upsample(self.outputs[2]), upsample(upsample(self.outputs[1])), upsample(upsample(upsample(self.outputs[0])))], 1)
...
class DepthNet(nn.Module):
    ...
    def __init__(self, num_layers=18, pretrained=True):
        ...
        self.HorNet = HorNet()
        # checkpoint = torch.load('models/hornet_tiny_7x7.pth')
        # self.HorNet.load_state_dict(checkpoint)
